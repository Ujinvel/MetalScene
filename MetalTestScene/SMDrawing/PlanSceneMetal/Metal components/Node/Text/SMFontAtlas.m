...
- (instancetype)initWithFont:(UIFont *)font textureSize:(NSInteger)textureSize {
    if ((self = [super init])) {
      ...
      _spread           = [self estimatedLineWidthForFont:font] / 2;
      _glyphDescriptors = [NSMutableArray array];
      _textureSize      = textureSize;
      
      if ([self createTextureData] == NO) return nil;
    }
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    if ((self = [super init])) {
      NSString *fontName = [aDecoder decodeObjectForKey:cFontNameKey];
      CGFloat fontSize   = [aDecoder decodeFloatForKey:cFontSizeKey];
      CGFloat spread     = [aDecoder decodeFloatForKey:cFontSpreadKey];

      // Encountered invalid persisted font (invalid font name or size)
      if (fontName.length == 0 || fontSize <= 0) return nil;
      
      _parentFont       = [UIFont fontWithName:fontName size:fontSize];
      _fontPointSize    = fontSize;
      _spread           = spread;
      _glyphDescriptors = [aDecoder decodeObjectForKey:cGlyphDescriptorsKey];
      
      // Encountered invalid persisted font (no glyph metrics)
      if (_glyphDescriptors == nil) return nil;
      
      NSInteger width  = [aDecoder decodeIntForKey:cTextureWidthKey];
      NSInteger height = [aDecoder decodeIntForKey:cTextureHeightKey];

      // Encountered invalid persisted font (non-square textures aren't supported)
      if (width != height) return nil;
      
      _textureSize = width;

      _textureData = [aDecoder decodeObjectForKey:cTextureDataKey];

      // Encountered invalid persisted font (texture data is empty)
      if (_textureData == nil) return nil;
    }
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    [aCoder encodeObject:self.parentFont.fontName forKey:cFontNameKey];
    [aCoder encodeFloat:self.fontPointSize forKey:cFontSizeKey];
    [aCoder encodeFloat:self.spread forKey:cFontSpreadKey];
    [aCoder encodeObject:self.textureData forKey:cTextureDataKey];
    [aCoder encodeInt64:self.textureSize forKey:cTextureWidthKey];
    [aCoder encodeInt64:self.textureSize forKey:cTextureHeightKey];
    [aCoder encodeObject:self.glyphDescriptors forKey:cGlyphDescriptorsKey];
}
//-----------------------------------------------------------------------------------
...
#pragma mark - Auxiliary methods for determining the size

- (CGSize)estimatedGlyphSizeForFont:(UIFont *)font {
    CGSize exemplarStringSize = [cExampleString sizeWithAttributes:@{ NSFontAttributeName: font }];
    CGFloat averageGlyphWidth = ceilf(exemplarStringSize.width / cExampleString.length);
...
- (CGFloat)estimatedLineWidthForFont:(UIFont *)font {
    CGFloat estimatedStrokeWidth = [cStroke sizeWithAttributes:@{ NSFontAttributeName: font }].width;
...
- (CGFloat)estimatedLineWidthForFont:(UIFont *)font andString:(NSString *)string {
    CGFloat estimatedStrokeWidth = [string sizeWithAttributes:@{ NSFontAttributeName: font }].width;
...
- (uint8_t *)createAtlasForFont:(UIFont *)font width:(NSInteger)width height:(NSInteger)height {
    ...
    CGContextRef context       = CGBitmapContextCreate(imageData, width, height, cBitsPerComponent, width, colorSpace, bitmapInfo);
    ...
    for (CGGlyph glyph = 0; glyph < fontGlyphCount; ++glyph) {
      ...
      CGFloat glyphOriginX = origin.x - boundingRect.origin.x + (glyphMargin / 2);
      CGFloat glyphOriginY = origin.y + (glyphMargin / 2);
      ...
      SMGlyphDescriptor *descriptor  = [SMGlyphDescriptor new];
...
#define nearestpt(_x, _y) boundaryPointMap[(_y) * width + (_x)]
    ...
    for (long y = 0; y < height; ++y) {
      for (long x = 0; x < width; ++x) {
        if (!image(x, y)) distance(x, y) = -distance(x, y);
      }
    }
...
- (float *)createResampledData:(float *)inData
    ...
    if (width % scaleFactor != 0 && height % scaleFactor != 0) return nil;
...
- (BOOL)createTextureData {
    // Requested font atlas texture size cFontAtlasSizeMax must be smaller than intermediate texture size self.textureSize
    if (cFontAtlasSizeMax <= self.textureSize) return NO;
    // Requested font atlas texture size cFontAtlasSizeMax does not evenly divide intermediate texture size self.textureSize
    if (cFontAtlasSizeMax % self.textureSize != 0) return NO;
    ...
    uint8_t *atlasData = [self createAtlasForFont:self.parentFont
                                            width:cFontAtlasSizeMax
                                           height:cFontAtlasSizeMax];
  
    NSInteger scaleFactor = cFontAtlasSizeMax / self.textureSize;
    ...
    CGFloat spread = [self estimatedLineWidthForFont:self.parentFont] / 2;
